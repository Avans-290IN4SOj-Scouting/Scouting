...
@section('content')
        ...
        <form action="{{ route('product.createProduct') }}" method="POST" enctype="multipart/form-data">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
                ...
                <div class="space-y-4">
                    <!-- Product Name Field -->
                    <!-- Product Name Field -->
                    @include('partials._input', ['label' => 'Product Naam', 'id' => 'product-name', 'type' => 'text', 'placeholder' => 'Product Naam', 'name' => 'name'])
                    <!-- Product Price Field -->
                    @include('partials._price-input', ['label' => 'Product Prijs', 'id' => 'product-price', 'placeholder' => 'Prijs', 'name' => 'priceForSize'])
                            ...
                            <div id="size-price-inputs" class="hidden">
                                ...
                                <div id="specific-size-prices">
                                    ...
                                    <div class="flex items-center space-x-4">
                                        ...
                                        <input type="number" placeholder="Prijs" class="w-full px-4 py-2 border border-gray-300 rounded-md specific-size-price" name="priceForSize[Small]">
                                    </div>
                                    <div class="flex items-center space-x-4">
                                        <span class="text-gray-600">Medium (M): €</span>
                                        <input type="number" placeholder="Prijs" class="w-full px-4 py-2 border border-gray-300 rounded-md specific-size-price" name="priceForSize[Medium]">
                                    </div>
                                    <div class="flex items-center space-x-4">
                                        <span class="text-gray-600">Large (L): €</span>
                                        <input type="number" placeholder="Prijs" class="w-full px-4 py-2 border border-gray-300 rounded-md specific-size-price" name="priceForSize[Large]">
                                    </div>
                                    <div class="flex items-center space-x-4">
                                        <span class="text-gray-600">Extra Large (XL): €</span>
                                        <input type="number" placeholder="Prijs" class="w-full px-4 py-2 border border-gray-300 rounded-md specific-size-price" name="priceForSize[Extra Large]">
                                    </div>
                    ...
                    <!-- Select -->
                    <!-- Select -->
                    @include('partials._select', ['label' => 'Scouting groepen', 'placeholder' => 'Select scouting groepen', 'options' => ['Bevers', 'Gidsen', 'Kabouters', 'Leiders', 'Verkenners', 'Waterwerk', 'Welpen'], 'name' => 'groups'])

                    <!-- Product Description Field -->
                    <label for="product-description" class="block text-gray-700 font-semibold">Product Beschrijving</label>
                    <div>
                        <textarea id="product-description" name="description" rows="4" class="w-full px-4 py-2 rounded-md shadow-md focus:outline-none focus:ring focus:border-blue-500 transition duration-300"></textarea>
                    </div>
    ...
    <script>
        function addCustomSizeInput() {
            ...
            sizeInput.classList.add('w-full', 'px-4', 'py-2', 'border', 'border-gray-300', 'rounded-md', 'specific-size-price');
