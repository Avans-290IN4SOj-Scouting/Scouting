            ...
            <form action="{{ route('product.createProduct') }}" method="POST" enctype="multipart/form-data">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
                    ...
                    <div class="space-y-4">
                        ...
                        @if (!empty($productViewModel->sizesWithPrices))
                            <p>Sizes with Prices:</p>
                            <ul>

                            </ul>
                        @else
                            <p>No sizes with prices available for this product.</p>
                        @endif
                            ...
                            <div id="size-price-options" class="space-y-4">
                                ...
                                <div id="size-price-inputs" class="hidden">
                                    ...
                                    <div id="specific-size-prices">
                                        ...
                                        @foreach ($productViewModel->sizesWithPrices as $sizeWithPrice)
                                            <div class="flex items-center space-x-4">
                                                <span class="text-gray-600">{{ $sizeWithPrice['size'] }}: €</span>
                                                <input type="number" placeholder="Prijs"
                                                    class="w-full px-4 py-2 border border-gray-300 rounded-md specific-size-price"
                                                    value="{{ $sizeWithPrice['price'] ?? '' }}" name="priceForSize[Medium]">
                                            </div>
                                        @endforeach
                        ...
                        @include('partials._select', [
                            'label' => 'Scouting product types',
                            'placeholder' => 'Select scouting product types',
                            'options' => [
                                'Bevers',
                                'Gidsen',
                                'Kabouters',
                        ...
                        <p>Category: {{ $productViewModel->category }}</p>
                            ...
                            <label for="af-submit-app-upload-images" id="drop-area"
                                ...
                                <div class="flex justify-center items-center containerMaxH">
                                    ...
                                    @if (!empty($product->image_path))
                                        <img id="file-image" src="{{ $product->image_path }}" alt="Preview"
                                            class="containerMaxH">
                                    @else
                                ...
                                <button id="remove-image" class="hidden mt-2 bg-red-500 text-white px-4 py-2 rounded-md">
                                    Delete Image
                                </button>
                            </label>
                        </div>
