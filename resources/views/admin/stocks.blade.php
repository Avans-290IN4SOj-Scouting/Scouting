            ...
            <div id="accordion-item-{{$loop->index + 1}}"
                ...
                <div class="accordion-header p-1.5 py-3">
                    ...
                    <p class="font-semibold text-lg dark:text-white">
                        {{$product->name . ', ' . $product->type->type . ' ('. $product->variety->variety .')'}}
                    </p>
                </div>
                <div class="accordion-content ps-6 py-2.5 space-x-1">
                    <form method="POST"
                          action="{{ route('manage.stocks.update', ['product' => $product->id, 'type' => $product->type_id]) }}">
                        @csrf
                        @method('PUT')
                        <div>
                            @foreach ($product->productSizes as $productSize)
                                <div
                                    ...
                                    <div class="border-t sm:border-t-0 sm:border-s border-gray-300 dark:border-gray-700">
                                    </div>
                                    <input
                                        autocomplete="off"
                                        name="size-{{strtolower($productSize->size)}}-{{$loop->parent->index + 1}}-{{$loop->index + 1}}"
                                        id="size-{{strtolower($productSize->size)}}-{{$loop->parent->index + 1}}-{{$loop->index + 1}}"
                                        ...
                                        type="text"
                                        value="{{ $product->stocks->where('product_size_id', $productSize->id)->where('product_type_id', $product->type_id)->first()->amount ?? 0 }}">
                                </div>
                            @endforeach
                            <button type="submit"
